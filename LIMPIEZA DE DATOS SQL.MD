...
# PASO 2  Cambiar de nombre la columna 
...
# PASO 3 Eliminación de valores nulos — columna Income
...
Las respuestas a las campañas (1 a 5), las quejas (Complain) y la respuesta final (Response) se configuraron técnicamente como BIT(1).

```sql
ALTER TABLE pro_mark.marketing_campaign_clean 
MODIFY COLUMN AcceptedCmp1 BIT(1),
MODIFY COLUMN AcceptedCmp2 BIT(1),
MODIFY COLUMN AcceptedCmp3 BIT(1),
MODIFY COLUMN AcceptedCmp4 BIT(1),
MODIFY COLUMN AcceptedCmp5 BIT(1),
MODIFY COLUMN Complain BIT(1),
MODIFY COLUMN Response BIT(1);
```
...
Siempre busco refinar mi flujo de trabajo y mis habilidades técnicas. Valoraría mucho cualquier comentario o sugerencia respecto al proceso de **Limpieza de Datos (ETL)** .

Ya sean consejos de optimización, una forma más escalable de manejar el modelo de datos o un enfoque diferente para las visualizaciones, ¡tus ideas son más que bienvenidas!
